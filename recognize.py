...
def chinese_hand_write_cnn():
    ...
    w_c1 = tf.Variable(tf.random.uniform([3, 3, 1, 32], minval=-0.01, maxval=0.01))
    ...
    w_c2 = tf.Variable(tf.random.uniform([3, 3, 32, 64], minval=-0.01, maxval=0.01))
    ...
    w_d = tf.Variable(tf.random.uniform([8 * 32 * 64, 1024], minval=-0.01, maxval=0.01))
    ...
    w_out = tf.Variable(tf.random.uniform([1024, 140], minval=-0.01, maxval=0.01))
    ...
    for e in range(50):
        for i in range(num_batch):
            ...
            with summary_writer.as_default():
                ...
                tf.summary.scalar("accuracy", accuracy_metric.result().numpy(), step=step)

            print(step, loss.numpy(), accuracy_metric.result().numpy())
            accuracy_metric.reset_states()

            if step % 100 == 0:
                # 计算准确率
                logits = output(text_data_x[:500], training=False)
                acc = accuracy_metric(text_data_y[:500], logits).numpy()
                print(step, acc)

    # 保存模型
    output.save_weights("model_h5")
